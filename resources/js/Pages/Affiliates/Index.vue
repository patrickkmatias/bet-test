    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                ...
                <PrimaryButton v-if="!user.affiliate" @click="viewEditForm = !viewEditForm">
        ...
        <div class="py-12">
            <div class="grid gap-6 mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div v-if="viewEditForm && !user.affiliate" class="p-6 flex flex-wrap overflow-hidden bg-white shadow-sm sm:rounded-lg">
                ...
                <div class="p-6 grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <Card v-for="a in affiliates" >
                        <CardHeader>
                            <CardTitle>{{ a.user.name }}</CardTitle>
                            <CardDescription>{{ a.cpf }}</CardDescription>
                            <CardDescription>{{ a.phone_number }}</CardDescription>
                        </CardHeader>
                    </Card>
                </div>
            </div>
